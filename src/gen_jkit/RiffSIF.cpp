...
BOOL CRiffSIF::SetField(char id1,char id2,char id3,char id4,const _TCHAR *szData)
{
...
DWORD CRiffSIF::GetTotalFieldSize()
{
	...
	{
		CString str = CStringToMbs(*(&p->second));
		DWORD len = strlen((char *)(LPCTSTR )str) + 1;
...
DWORD CRiffSIF::Load(const _TCHAR *szFileName,char id1,char id2,char id3,char id4)
{
	int	dwWin32errorCode = ERROR_SUCCESS;
	HANDLE hFile;
	DWORD dwRet;
	DWORD dwTotalSize;
	DWORD dwFileSize;
	...
	if(FindChunk(hFile,dwFileSize,MMIO_FINDLIST,mmioFOURCC('I','N','F','O'),&dwSize,FALSE) && (dwSize > 8))
	{
		...
		while(dwRemainSize > 0)
		{
			...
			if(data[size-1] == '\0')
			{
				m_fields.insert(pair<FOURCC,CString>(id,CString(data)));
			}
			else
			{
...
/*
	ret:	-1 = 更新失敗
			-2 = ファイルサイズが2Gを超過している
*/
DWORD CRiffSIF::Save(HWND hWnd,const _TCHAR *szFileName)
{
	hWnd;
	int	dwWin32errorCode = ERROR_SUCCESS;
	HANDLE hFile;
...
//	LONG lAddressHeight;
	...
	//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
	//LIST-INFOを終端に追加
	SetFilePointer(hFile,-(int )dwInfoSize,NULL,FILE_CURRENT);
	dwSize = dwInfoSize - 8;
	id = FOURCC_LIST;
	...
	while(p != m_fields.end())
	{
		...
		if(len > 1)
		{
			...
			WriteFile(hFile,(char *)(LPCTSTR )str,len,&dwRet,NULL);
			if(len%2)
			{
